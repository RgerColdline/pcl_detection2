...
class CloudAccumulator
{
  public:
    ...
    CloudAccumulator(ros::NodeHandle &nh, ros::NodeHandle &pnh) : nh_(nh), pnh_(pnh) {
        ...
        // ====== 输入源选择 ======
        std::string cloud_source = "livox";
        pnh_.param("cloud_source", cloud_source, cloud_source);
        cloud_source_ = cloud_source;

        if (cloud_source_ == "fastlio") {
            cloud_registered_sub_ =
                nh_.subscribe("/fastlio_map", 1, &CloudAccumulator::cloudRegisteredCallback, this);
            fastlio_odom_sub_ =
        ...
        pnh_.param("pre_register_roi/x_min", pre_roi_x_min_, roi_x_min_);
        pnh_.param("pre_register_roi/x_max", pre_roi_x_max_, roi_x_max_);
        pnh_.param("pre_register_roi/y_min", pre_roi_y_min_, roi_y_min_);
        pnh_.param("pre_register_roi/y_max", pre_roi_y_max_, roi_y_max_);
        pnh_.param("pre_register_roi/z_min", pre_roi_z_min_, roi_z_min_);
        pnh_.param("pre_register_roi/z_max", pre_roi_z_max_, roi_z_max_);
    ...
    void cloudCallback(const livox_ros_driver2::CustomMsg::Ptr &livox_msg) {
            ...
            if (!downsampled_registration_map_cloud_->empty()) {
                    ...
                    static_cast<size_t>(register_min_submap_points_))
                {
                    ...
                    if (registration_accepted) {
                        // ICP用ROI点云配准得到修正矩阵，但全量点云按修正变换写入地图
                        pcl::transformPointCloud(*predicted_livox_cloud_, *aligned_livox_cloud_,
                                                 correction_delta);
                        last_map_pose_ = correction_delta * predicted_pose;
                    }
                    else {
        ...
        if (timing_enable_) timing.mark("publish");
        ...
        if (timing_enable_) timing.mark("ring_detect");
        ...
        if (timing_enable_) timing.mark("pillar_detect");

        if (timing_enable_) logTimingReport(timing);
    }
  ...
  private:
    ...
    // ============================================================
    //  FAST-LIO2 新管线（cloud_source == "fastlio" 时启用）
    // ============================================================

    void fastlioOdomCallback(const nav_msgs::Odometry::ConstPtr &msg) {
        last_fastlio_pose_.block<3, 3>(0, 0) =
    ...
    void cloudRegisteredCallback(const sensor_msgs::PointCloud2::ConstPtr &msg) {
        ...
        // 1. sensor_msgs → PCL
        PointCloudPtrT raw_cloud(new PointCloudT);
  ...
  private:
    ...
    float pre_roi_z_min_, pre_roi_z_max_;
